...
export function createBackroomMcpServer(props: McpGrantProps) {
  ...
  registerTool(
    'get_validation_retry',
    {
      ...
      description:
        ...
        'Each ticket also carries why it ended: silently_expired (the lease ran out with nothing reported about that attempt), failure_reason and failed_at (history, not current state — a manual reissue preserves the last report), slot_id, purpose (canonical_quorum or continual_retest), first_reported_at (null means the validator never advertised the slot as active), and infra_retry_grants. infra_retry_grants is historical evidence from deployments that minted automatic infrastructure grants; it no longer authorizes a lease. provider_outage is the relay provider circuit (state, last_failure_at, last_error_code, closed_at = last recovery) when a remaining slot was parked by provider_outage_parked; provider_outage_blocks_retry means that circuit is still open, so recommended_action is not retry and a grant would be parked again. Every current failure parks after one attempt until retry_validator_evaluation or retry_validator_evaluations is issued manually. silently_expired reads null against a platform that predates #515. If a lease was ended by the platform rather than by a validator report, list_lease_revocations carries the verdict and its evidence. Requires backroom:read and exposes no miner source.',
  ...
  registerTool(
    'retry_validator_evaluation',
    {
      ...
      description:
        'Restore only the exhausted validation slots needed for quorum after an operator verifies validator-owned infrastructure failure. Preserves scores, screening verdicts, artifacts, payments, ownership, and all ticket history. This is not rescreening and acts on one agent only. Refused (409) while provider_outage_blocks_retry is true unless acknowledgeProviderOutage=true: the provider circuit that parked the slots is still open. Requires backroom:write.',
