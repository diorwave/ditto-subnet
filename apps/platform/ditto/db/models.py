...
class CodingCertificationInferenceGrant(Base):
    """One locked Luna capability bound to one claimed certification lease."""

    __tablename__ = "coding_certification_inference_grants"

    grant_id: Mapped[UUID] = mapped_column(
        SaUUID(as_uuid=True), primary_key=True, default=uuid4
    )
    lease_id: Mapped[UUID] = mapped_column(SaUUID(as_uuid=True), nullable=False)
    validator_hotkey: Mapped[str] = mapped_column(Text, nullable=False)
    case_id: Mapped[str] = mapped_column(Text, nullable=False)
    profile_capability_id: Mapped[str] = mapped_column(Text, nullable=False)
    inference_grant_sha256: Mapped[str] = mapped_column(Text, nullable=False)
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ...
            ["coding_certification_leases.lease_id"],
            ondelete="CASCADE",
            name="coding_certification_inference_grants_lease_fkey",
        ),
        UniqueConstraint(
            "lease_id",
            name="coding_certification_inference_grants_lease_key",
        ),
        UniqueConstraint(
            "grant_id",
            "lease_id",
            name="coding_certification_inference_grants_grant_lease_key",
        ),
        CheckConstraint(
            "generation BETWEEN 0 AND 2147483647 "
            "AND request_budget BETWEEN 1 AND 256 "
            "AND prompt_token_budget BETWEEN 1 AND 2000000 "
            "AND completion_token_budget BETWEEN 1 AND 250000 "
            "AND cost_budget_usd_micros BETWEEN 1 AND 100000000",
            name="coding_certification_inference_grants_bounds_check",
        ),
        CheckConstraint(
            "request_count BETWEEN 0 AND request_budget "
            "AND prompt_tokens >= 0 AND completion_tokens >= 0 "
            "AND cost_usd_micros >= 0 AND active_requests BETWEEN 0 AND 1",
            name="coding_certification_inference_grants_accounting_check",
        ),
        CheckConstraint(
            "inference_grant_sha256 ~ '^[0-9a-f]{64}$' "
            "AND (bearer_digest IS NULL OR bearer_digest ~ '^[0-9a-f]{64}$') "
            ...
            "AND (broker_public_key IS NULL OR "
            "broker_public_key ~ '^[A-Za-z0-9_-]{43}$')",
            name="coding_certification_inference_grants_crypto_check",
        ),
        CheckConstraint(
            "model = 'openai/gpt-5.6-luna' "
            "AND provider_api = 'openrouter' "
            ...
            "AND provider_cache_policy = 'disabled_v1' "
            "AND reasoning_effort = 'medium'",
            name="coding_certification_inference_grants_route_check",
        ),
        CheckConstraint(
            "status IN ('pending', 'active', 'revoked', 'exhausted') "
            "AND ((status = 'pending' AND generation = 0 "
            ...
            "AND bearer_digest IS NULL AND broker_public_key IS NULL)) "
            "AND ((status = 'revoked') = (revoked_at IS NOT NULL))",
            name="coding_certification_inference_grants_state_check",
        ),
        CheckConstraint(
            "octet_length(case_id) BETWEEN 1 AND 256 "
            "AND octet_length(profile_capability_id) BETWEEN 1 AND 256 "
            "AND case_id !~ '[[:space:][:cntrl:]]' "
            "AND profile_capability_id !~ '[[:space:][:cntrl:]]' "
            "AND expires_at > created_at AND weight_eligible = false",
            name="coding_certification_inference_grants_shadow_check",
        ),
        Index(
            "coding_certification_inference_grants_validator_expiry_idx",
            "validator_hotkey",
            "expires_at",
            postgresql_where=text("status IN ('pending', 'active')"),
        ),
...
class CodingCertificationInferenceRequest(Base):
    """One durable canary request reservation and its trusted settlement."""

    __tablename__ = "coding_certification_inference_requests"
